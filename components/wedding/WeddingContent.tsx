...
export function WeekendEvents({ compact = false }: { compact?: boolean }) {
  return <div className={styles.events}>
    {weekendEvents.map((event) => <article key={event.day} className={styles.event}>
      <div className={styles.eventTop}><p className={styles.eyebrow}>{event.day}, {event.date}</p><InkDoodle kind={event.day === "Friday" ? "cheers" : event.day === "Saturday" ? "smores" : "sunglasses"} className={styles.scheduleDoodle} size={54}/></div>
      <h3>{event.name}</h3>
...
export function NextPage({ href, label }: { href: string; label: string }) {
  return <div className={styles.nextPage}><p className={styles.eyebrow}>A little more planning</p><Link href={href}>{label}<svg className={styles.nextPageArrow} viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M5 11 11 5M5 5h6v6"/></svg></Link></div>;
}
