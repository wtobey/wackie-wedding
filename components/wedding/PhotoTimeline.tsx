...
const Photo = memo(function Photo({ photo, large = false }: { photo: TimelinePhoto; large?: boolean }) {
  ...
  return <Image src={photo.imageUrl} alt={photo.caption || "A memory from Will and Jackie’s photo collection"} fill unoptimized sizes={large ? "90vw" : "(max-width: 600px) 65vw, 300px"} loading={large ? "eager" : "lazy"} onError={() => setFailed(true)}/>;
});
