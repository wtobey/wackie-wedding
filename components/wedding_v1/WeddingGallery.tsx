...
export default function WeddingGallery() {
  ...
  useEffect(() => {
    ...
    fetch('/api/wedding/photos?collection=gallery', { signal: controller.signal, cache: 'no-store' })
      ...
      .then((rows: ManagedPhoto[]) => setState({ photos: rows.map((row, index) => ({ id: index + 1, caption: row.caption, crop: row.crop, alt: row.alt, photoDate: row.photoDate, imageUrl: row.imageUrl, thumbnailUrl: row.thumbnailUrl, sortOrder: index })), loading: false, error: false }))
