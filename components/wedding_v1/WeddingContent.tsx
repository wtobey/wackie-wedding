...
import { weekendEvents } from "@/data/wedding_v1";
...
export function WeekendEvents({ compact = false }: { compact?: boolean }) {
  return <div className={styles.events}>
    {weekendEvents.map((event) => <article key={event.day} className={styles.event}>
      <div className={styles.eventTop}><p className={styles.eyebrow}>{event.day}, {event.date}</p></div>
      <h3>{event.name}</h3>
      {!compact && <p className={styles.eventLocation}>{event.location}</p>}
      <div className={styles.eventDescription}>{event.schedule.map((line) => <p key={line}>{line}</p>)}</div>
