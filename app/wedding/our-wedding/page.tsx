...
export default function OurWedding() {
  return <div className={`${styles.pageWrap} ${styles.itineraryPage}`}>
    ...
    <section className={styles.venueAddress}><Doodle kind="trees" className={styles.addressDoodle}/><div><p className={styles.eyebrow}>Venue Information and Onsite Accommodations</p><h2>{wedding.venue}</h2><address>{wedding.address}<br/>{wedding.town}</address><a className={styles.textLink} href={wedding.mapsUrl} target="_blank" rel="noreferrer">Open in maps</a></div><p>{weddingTravel.stay} Please see the <Link href="/wedding/accommodations" style={{ textDecoration: "underline" }}>Accommodations</Link> page for more details.</p></section>
  </div>;
}
