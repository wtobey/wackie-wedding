...
import { signedMediaUrl, storageConfigured } from '@/lib/wedding-admin/storage';
...
export async function GET(request: Request, { params }: { params: Promise<{ name: string }> }) {
  ...
  try {
    ...
    const published = library.photos.some(photo => photo.imageUrl === `/api/wedding/media/${name}` && photo.included);
    if (!published && !await isAdmin()) return new Response(null, { status: 404 });
    const size = new URL(request.url).searchParams.get('size');
    if (storageConfigured) {
      const url = await signedMediaUrl(name, size);
    ...
    return new Response(new Uint8Array(bytes), { headers: { 'Content-Type': 'image/webp', 'Cache-Control': published ? 'private, max-age=300' : 'private, no-store', 'X-Content-Type-Options': 'nosniff' } });
